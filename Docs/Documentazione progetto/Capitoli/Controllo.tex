...
Abbiamo racchiuso in figura ~\ref{fig:OPCUA_diagram} il funzionamento di massima del codice lato server: codice che siamo andati a testare utilizzando un'apposita app per smartphone Android (\href{https://play.google.com/store/apps/details?id=com.prosysopc.ua.android2&hl=it}{OPC-UA Android client}); per quanto riguarda invece l'effettiva implementazione, abbiamo riportato nell'Appendice ~\ref{cod:Appendice_opcua} l'intero codice prodotto, per la gestione della comunicazione OPC-UA.
