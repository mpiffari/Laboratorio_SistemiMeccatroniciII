...
\begin{center}
	...
	\begin{cases}
		\begin{array}{c}
			\mathbf{f_1} \to {\dot{x_1} \left(t\right)} =\dot{\phi}= {x_3 \left(t\right)} =0\text{ (poichè all'equilibrio)}\\
			\mathbf{f_2} \to {\dot{x_3} \left(t\right)} = {\ddot{\phi} \;}\\
			\mathbf{f_3} \to {\dot{x_2} \left(t\right)} =\dot{\theta} = {x_4 \left(t\right)} =0\text{ (poichè all'equilibrio)}\\
...
\begin{itemize}
	...
	\item D $\rightarrow$ $[2n \times 1]$: anche in questo caso la dimensione è collegata alla scelta fatta per la matrice C;
\end{itemize}
